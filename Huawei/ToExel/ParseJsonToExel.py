...
from search import is_absent, Resources, huawei, stripJSON
...
def _unique_by_sn(components, prefix):
    ...
    for component in _ensure_list(components):
        if not isinstance(component, dict) or is_absent(component):
            continue
...
def write_proc_info(ws, server_data, start_row, start_col):
    ...
    # An explicit list of empty sockets must not be replaced by summary CPUs.
    if _ensure_list(server_data.get("Processors")):
        return (start_row, 5)
...
def write_RAID_info(ws, server_data, start_row, start_col):
    ...
    # One physical battery can supply several controllers.
    for battery in _unique_by_sn(server_data.get("StorageBatteries"), "BAT"):
        start_row = _write_component_row(
            ws, start_row,
...
def write_other_info(ws, server_data, start_row, start_col):
    if not _is_invalid_identifier(server_data.get("SDCard")) and not is_absent(server_data.get("SDCard")):
        start_row = _write_component_row(
            ws,
            start_row,
    ...
    for field, prefix, name in (("StorageEnclosures", "ENCLOSURE", "Дисковая полка"),
                                ("Boards", "BOARD", "Плата"),
                                ("Fans", "FAN", "Вентилятор"),
                                ("USBDevices", "USB", "USB устройство")):
        for component in _unique_by_sn(server_data.get(field), prefix):
            start_row = _write_component_row(
                ws, start_row,
                ...
                comment=component.get("Comment") or component.get("Location"),
                telemetry=component,
            )
...
def write_server_info(ws, server_data, start_row, current_col):
    ...
    versions = []
    for label, field in (("BIOS", "BiosVersion"), ("iBMC", "iBMCVersion")):
...
def parse_data(raw_data):
    if not isinstance(raw_data, dict):
        return None
    resources = Resources(raw_data)
    system = resources.resolve(resources.system_path)
    if not system or ("huawei" not in str(system.get("Manufacturer", "")).lower() and not huawei(system)):
        return None
