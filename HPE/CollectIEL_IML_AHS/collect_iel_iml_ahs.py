...
# Класс для сбора ТОЛЬКО логов IEL и IML с серверов через Redfish
class ServerInfoCollector:
    """
    - IML (Integrated Management Log) находится под /redfish/v1/Systems/<id>/LogServices/IML
    - IEL (iLO Event Log)          находится под /redfish/v1/Managers/<id>/LogServices/IEL

    Остальные ресурсы (Chassis, прочие LogServices и т.д.) не собираются.
    AHS-файл скачивается отдельно в process_server (это не Redfish-ресурс).
    """
    def __init__(self, client):
        ...
        self.collected_data = {}
        # Разрешаем только навигацию к Systems/Managers и веткам IEL/IML
        self.INCLUDE_PATTERNS = [
            # Коллекции и их члены — нужны для навигации к LogServices и серийному номеру
            r"/redfish/v1/Systems(/[^/]+)?$",
            r"/redfish/v1/Managers(/[^/]+)?$",
            # Коллекции LogServices и конкретные сервисы логов
            r"/redfish/v1/Systems/[^/]+/LogServices(/[^/]+)?$",
            r"/redfish/v1/Managers/[^/]+/LogServices(/[^/]+)?$",
            # Записи IML (под Systems) и IEL (под Managers) со всеми вложениями
            r"/redfish/v1/Systems/[^/]+/LogServices/IML(/.*)?$",
            r"/redfish/v1/Managers/[^/]+/LogServices/IEL(/.*)?$",
        ]
        # Исключаем любые LogServices, кроме IML (для Systems) и IEL (для Managers)
    ...
    def collect_resources(self, output_file=None):
        # Собирает ресурсы IEL/IML с сервера через Redfish API
        root_url = self.normalize_url("/redfish/v1")
        to_process = [root_url]
        visited = set()
        file_handle = open(output_file, "w", encoding="utf-8") if output_file else None
        try:
            self.client._log_info("Начало сбора логов IEL/IML")
            print(f"Сбор логов IEL/IML начат ({self.client.server_ip})")
            while to_process:
            ...
            self.client._log_info("Сбор логов IEL/IML завершен")
            print(f"Сбор логов IEL/IML завершен ({self.client.server_ip})")
        except Exception as e:
            # Логируем ошибки при сборе ресурсов
            self.client._log_error(f"Ошибка при сборе логов: {type(e).__name__}: {str(e)}\nПолный стек: {traceback.format_exc()}")
...
# Обрабатывает данные одного сервера: собирает IEL/IML и скачивает AHS
def process_server(server, output_dir):
    ...
    if client.client is None or not client.login():
        ...
        return False
    try:
        # Собираем ТОЛЬКО логи IEL и IML
        collector = ServerInfoCollector(client)
        server_info_file = os.path.join(temp_dir, "IEL_IML_logs.json")
        collector.collect_resources(output_file=server_info_file)

        # Извлекаем серийный номер для имени папки/AHS-файла
        serial_number = None
        ...
        # Запрашиваем и сохраняем AHS-файл (Active Health System).
        # Это не Redfish-ресурс — скачивается напрямую с авторизованной сессией.
        ...
        for ahs_link in ahs_links_args:
            ...
            client._log_error("Не удалось получить AHS-файл")
...
def main():
    ...
    print("Запуск сбора логов IEL/IML/AHS (прямая авторизация в iLO)")
